declare namespace com {
    namespace sun {
        namespace java {
            namespace swing {
                namespace plaf {
                    namespace windows {
class WindowsTreeUI$WindowsTreeCellRenderer extends javax.swing.tree.DefaultTreeCellRenderer {
    public constructor(arg0: com.sun.java.swing.plaf.windows.WindowsTreeUI)
    public getTreeCellRendererComponent(arg0: javax.swing.JTree | javax.swing.JTree$$Lambda, arg1: java.lang.Object, arg2: boolean, arg3: boolean, arg4: boolean, arg5: int, arg6: boolean): java.awt.Component
    public static class: java.lang.Class<any>
}

                    }
                }
            }
        }
    }
}