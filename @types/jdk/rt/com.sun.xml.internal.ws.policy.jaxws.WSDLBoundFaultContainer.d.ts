declare namespace com {
    namespace sun {
        namespace xml {
            namespace internal {
                namespace ws {
                    namespace policy {
                        namespace jaxws {
                            class WSDLBoundFaultContainer implements com.sun.xml.internal.ws.api.model.wsdl.WSDLObject {
                                public constructor(arg0: com.sun.xml.internal.ws.api.model.wsdl.WSDLBoundFault, arg1: com.sun.xml.internal.ws.api.model.wsdl.WSDLBoundOperation)
                                public getLocation(): org.xml.sax.Locator
                                public getBoundFault(): com.sun.xml.internal.ws.api.model.wsdl.WSDLBoundFault
                                public getBoundOperation(): com.sun.xml.internal.ws.api.model.wsdl.WSDLBoundOperation
                                public static class: java.lang.Class<any>
                            }
                        }
                    }
                }
            }
        }
    }
}