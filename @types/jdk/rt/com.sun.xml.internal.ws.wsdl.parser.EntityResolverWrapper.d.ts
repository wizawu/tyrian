declare namespace com {
    namespace sun {
        namespace xml {
            namespace internal {
                namespace ws {
                    namespace wsdl {
                        namespace parser {
                            class EntityResolverWrapper implements com.sun.xml.internal.ws.api.wsdl.parser.XMLEntityResolver {
                                public constructor(arg0: org.xml.sax.EntityResolver | org.xml.sax.EntityResolver$$Lambda)
                                public constructor(arg0: org.xml.sax.EntityResolver | org.xml.sax.EntityResolver$$Lambda, arg1: boolean)
                                public resolveEntity(arg0: java.lang.String | string, arg1: java.lang.String | string): com.sun.xml.internal.ws.api.wsdl.parser.XMLEntityResolver$Parser
                                public static class: java.lang.Class<any>
                            }
                        }
                    }
                }
            }
        }
    }
}